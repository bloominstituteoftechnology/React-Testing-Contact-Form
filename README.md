...
**Test Runner**
- [1] In a new terminal window, run `npm test`
- [1] With each saved change in your editor, the test runner will re-run all the tests
- [1] **IMPORTANT** If a test fails, use the test runner's error messages to find out why it is failing
...
- [1] You may have noticed that the form validation for one of the inputs is a little off...
- [1] Write a test for the "expected" behavior (it will fail because something in the component is doing something unexpected)
- [1] Confirm that the test fails, because of the "unexpected" behavior
- [1] Look at the code in the project to find what is causing the behavior
- [1] Fix the code so that your new test passes
- [1] Celebrate that your test helped you find and fix a bug!
